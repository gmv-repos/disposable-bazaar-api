...
class BankAccountController extends Controller
{
    ...
    {
        $bankList = BankAccount::where('deleted', 0)->where('type', 1)->get();
        return view('adminPanel.bank_account.bank_list')->with(compact('bankList'));
    }
    public function bankStore(Request $request)
    {
        $bankAccount = new BankAccount();
        $bankAccount->account_name = $request->bank_name;
        $bankAccount->type = 1;
    ...
    public function bankActive($id)
    {
        ...
        $bankAccount->status = 1; // Active
        $bankAccount->save();

        // Redirect back with a success message
        return redirect()->route('admin.bank.list')->with('success', 'Bank Account activated successfully!');
    }
    ...
    public function bankInactive($id)
    {
        ...
        // Directly set the status field manually
        $bankAccount->status = 0; // Inactive
        $bankAccount->save();

        // Redirect back with a success message
        return redirect()->route('admin.bank.list')->with('success', 'Bank Account deactivated successfully!');
    }
}
