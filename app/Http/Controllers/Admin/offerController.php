...
class offerController extends Controller
{
    ...
    public function offerProductList(Request $request)
    {
        ...
        $offerProductList = Offer_product_list::where('status', 1)
            ->where('deleted', '0')
            ->where('offer_id', $request->id)
            ->get();
        $productList = Product::where('status', 1)->where('deleted', '0')->get();
        return view('adminPanel.offer.offer_product_list')->with(
            compact('offerList', 'productList', 'offerProductList', 'offer', 'common_data'),
        );
    }
