...
class SupplierController extends Controller
{
    ...
    public function purchaseItemGet(Request $request)
    {
        ...
        return view('adminPanel.product_stock._purchase_product_item')
            ->with(compact('productinfo', 'discount'))
            ->render();
    }

    public function purchaseItemVariantGet(Request $request)
    {
        $productVariantInfo = ProductVariant::with('product', 'variant')->find($request->product_variant_id);
        ...
        return view('adminPanel.product_stock._purchase_product_item')
            ->with(compact('productVariantInfo', 'productinfo', 'discount'))
            ->render();
    }

    public function toggleStatus($supplier_id)
    {
        $supplier = Supplier::findOrFail($supplier_id);
        $supplier->status = !$supplier->status;
        $supplier->save();

        return redirect()->back()->with('success', 'Supplier Status Changed');
    }
}
