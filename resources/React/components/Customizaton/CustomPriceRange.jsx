...
const CustomPriceRange = ({ onFilter }) => {
    ...
    const [selectedCate, setSelectedCate] = useState(''); // A to Z selected by default
    const [selectedSize, setSelectedSize] = useState(''); // A to Z selected by default
    const [selectedOption, setSelectedOption] = useState(''); // A to Z selected by default
    const [selectedQuantity, setSelectedQuantity] = useState(''); // A to Z selected by default
    const [selectedRating, setSelectedRating] = useState(''); // A to Z selected by default
    const [searchTerm, setSearchTerm] = useState('');
    const [categoryId, setCategoryId] = useState(); // Store selected category ID
    const [packSize, setPackSize] = useState(); // Store selected category ID
    const [sizes, setsizes] = useState(); // Store selected category ID
    const [Rate, setRate] = useState([]); // Store selected category ID
    const [options, setOptions] = useState(); // Store selected category ID
    ...
    const handleMinChange = (e) => {
        ...
        setPriceFrom(value);
        onFilter({ price_from: value, price_to: priceTo, selected, category_Id: categoryId });
    };
    ...
    const handleMaxChange = (e) => {
        ...
        setPriceTo(value);
        onFilter({ price_from: priceFrom, price_to: value, selected, category_Id: categoryId });
    };

    // Handle sorting checkbox change
    const handleChange = (value) => {
        setSelected(value);
        onFilter({ price_from: priceFrom, price_to: priceTo, selected: value, category_Id: categoryId });
    };
            ...
            <form action="">
                ...
                <div className=" py-2">
                    <div className="flex flex-col gap-2">
                        <h3 onClick={() => setSort(!sort)} className="flex flex-row justify-between items-center text-[#9F9F9F] text-xl font-bazaar mt4"><p>Sort By</p><p>{sort ? <PiCaretUpThin /> : <PiCaretDownThin />}</p></h3>
                        {sort && (
                ...
                <hr className='my-2 border border-[#9F9F9F]' />
                <div className="py-2">
                    <h3 onClick={() => setCate(!cate)} className="flex flex-row justify-between items-center text-[#9F9F9F] text-xl font-bazaar mt4"><p>Products Categories</p><p>{cate ? <PiCaretUpThin /> : <PiCaretDownThin />}</p></h3>
                    {cate && (
                            ...
                            {categories.map((cate) => (
                                <>
                                    <label key={cate.id} className="flex items-center gap-3 cursor-pointer">
                                        ...
                                        <span>{cate.name.length > 26 ? `${cate.name.slice(0, 26)}...` : cate.name}</span>
                                    </label>
                                    {cate.subCategories.map((cat) => (
                                        <label key={cat.id} className="flex items-center gap-3 cursor-pointer">
                                            <FiCornerDownRight />
                                            <input
                                                type="checkbox"
                                                checked={selectedQuantities.includes(cat.id)}
                                                onChange={() => { handleCate(cat.id) }}
                                                className="hidden"
                                            />
                                            <span
                                                className={`custom-checkbox  ${selectedCategories.includes(cat.id) ? 'checked' : ''}`}
                                            >
