...
<body>
    <header class="clearfix">
        ...
        <div id="company" style="font-size: 16px" class="clearfix">
            <div style="font-size: 30px;font-weight: 700">PURCHASE</div>
            <div><span>Purchase</span>:#{{$purchase->purchase_code}}</div>
    ...
    <main>
        <table>
            ...
            <tbody>
                ...
                @php
                $sub = round($purchasedata->purchase_payable_amount);
                @endphp
                <tr>
                    <td class="service">{{$key+1}}</td>
                    <td class="desc">{{$purchasedata->productInfo->name ?? "N/A"}}</td>
                    <td class="desc">{{$purchasedata->brand->name ?? "N/A"}}</td>
                    
                    <td class="unit">
                        {{number_format($purchasedata->unit_cost,0)}}
                    </td>
                    <td class="qty">{{$purchasedata->total_qty}}</td>
                    <td class="total">{{number_format($purchasedata->purchase_payable_amount,0)}}</td>
                    <?php $subtotal += $sub; ?>
                </tr>
                @endforeach
                ...
                <tr>
                    ...
                    <td class="total">{{number_format($purchase->total_discount,0)}}</td>
                </tr>
                <tr>
                    <td colspan="4">Delivery Charges</td>
                    <td class="total">{{number_format($purchase->delivery_charges,0)}}</td>
                </tr>
                <tr>
                    <td colspan="4" class="grand total">GRAND TOTAL</td>
                    <td class="grand total">
                        {{number_format(($subtotal - $purchase->total_discount) + $purchase->delivery_charges,0)}}
                    </td>
                </tr>
                {{-- <tr>
                    <td colspan="5" class="grand total">TOTAL PAYED</td>
                    <td class="grand total">{{ number_format($purchase->total_paid,0) }}</td>
                </tr>
                <tr style="background:white;border:none">
                    <td colspan="5" class="grand total" style="background:white;border:none">TOTAL DUE</td>
                    <td class="grand total" style="background:white;border:none">
                        {{ number_format((($subtotal - $purchase->total_discount) + $purchase->delivery_charges) - $purchase->total_paid,0 ) }}
                    </td>
                </tr>  --}}
            </tbody>
        </table>
