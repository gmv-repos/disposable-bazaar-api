    ...
    <div class="page-content">
        <div class="card">
            <div class="card-body">
                ...
                <form id="multiActionForm" method="POST" action="{{ route('web.orders.multiActionForm') }}">
                    @csrf
                    <div class="row justify-content-between" id="formActionsRow" style="display: none">
                        <div class="col-md-5 d-flex gap-2">
                            ...
                            <button type="button" class="btn btn-success btn-sm"
                                onclick="submitMultiActionForm('riderAndPay')">
                                Allocate
                            </button>
                        </div>
                        <div class="col-md-3 d-flex gap-2">
                            ...
                            <button type="button" class="btn btn-info btn-sm w-50"
                                onclick="submitMultiActionForm('print')">Print</button>
                            <button type="button" class="btn btn-danger btn-sm w-50"
                                onclick="submitMultiActionForm('downloadPDF')">Download PDF</button>
                        </div>
                    </div>
                            ...
                            <tbody>
                                ...
                                @foreach ($sellList as $key => $sell)
                                    <tr>
                                        ...
                                        <td>
                                            {!! getOrderStatusBadge($sell->order_status) !!}
                                        </td>
...
@section('js')
    <script>
        $(document).ready(function() {
            ...
            // Initialize Select2
            $('#filter_rider').select2();

            // Select all checkboxes
            $('#select_all').on('change', function() {
                const checked = $(this).is(':checked');
                $('input[name="orderIds[]"]').prop('checked', checked);
                $('input[name="sellIds[]"]').prop('checked', checked);
                toggleMultiActionForm();
            });

            $(document).on('change', 'input[name="orderIds[]"], input[name="sellIds[]"]', function() {
                toggleMultiActionForm();
            });

            function toggleMultiActionForm() {
                const hasChecked = $('input[name="orderIds[]"]:checked, input[name="sellIds[]"]:checked').length >
                    0;
                $('#formActionsRow').toggle(hasChecked);
                $('#formAction').val('');
            }
        ...
        function submitMultiActionForm(action) {
            $('#formAction').val(action);
            $('#multiActionForm').trigger('submit')
        }
