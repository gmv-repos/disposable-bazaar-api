...
@foreach($getCashLedger as $gclRow)
    @php
        $paymentAmount = 0;
        $receiptAmount = 0;
        $amountOut = 0;
        ...
        switch ($gclRow->transaction_type) {
            case 2:
                $receiptAmount = $gclRow->receipt_amount;
                $balanceAmount += $receiptAmount;
                $totalReceipts += $receiptAmount;
                break;

            case 4:
                $paymentAmount = $gclRow->receipt_amount;
                $balanceAmount -= $paymentAmount;
                $totalPayments += $paymentAmount;
                break;

            case 5:
                $paymentAmount = $gclRow->expense_amount;
                $balanceAmount -= $paymentAmount;
                $totalPayments += $paymentAmount;
                break;

            case 21:
                $amountIn = $gclRow->amount_in;
                $balanceAmount += $amountIn;
                $totalAmountIn += $amountIn;
                break;

            case 22:
                $amountOut = $gclRow->amount_out;
                $balanceAmount -= $amountOut;
                $totalAmountOut += $amountOut;
                break;

            case 23:
                $extraInAmount = $gclRow->extra_trx_amount;
                $balanceAmount += $extraInAmount;
                $totalExtraIn += $extraInAmount;
                break;

            case 24:
                $extraOutAmount = $gclRow->extra_trx_amount;
