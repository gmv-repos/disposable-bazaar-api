                    ...
                    <div class="row p-1">
                        <div class="col-md-4">
                            <select name="sales_order_id" id="sales_order_id" class="form-control select2"
                                onchange="getSalesOrderInfo(this.value)">
                                <option value="">
                                    Select Sales Order Number
                                </option>
                                @foreach($salesOrderList as $sol)
                                    <option value="{{$sol->id}}">
                    ...
                    <div class="rightpos">
                        <div class="row d-flex justify-content-center align-items-center posTopbar">
                            ...
                            <div class="col-md-4 text-center">
                                <h3 class="text-white">NEW ORDER</h3>
                            </div>
                            <div class="col-sm-4 d-flex justify-content-end">
                                {{-- <span class="addCustomer" onclick="customeradd()">
                                    <i style="font-size: 18px;" class="lni lni-circle-plus"></i>
                                    &nbsp; Customer
                                </span> --}}
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="row justify-content-end align-items-center">
                                <div class="col-2">
                                    <select name="discount_type" class="form-select form-select-sm" id="distypeset"
                                        onchange="countTotal()">
                                ...
                                <div class="col-2">
                                    <input type="number" id="discountInput" oninput="countTotal()" class="form-control">

                                    <input type="hidden" name="hiddenDiscountAmount" id="hiddenDiscountAmount">
                            ...
                            <div class="row justify-content-end align-items-center mt-2">
                                <div class="col-2">
                                    <select class="form-select form-select-sm" onchange="invoiceAreaChange(this)">
                                        <option value="0">Select Area</option>
                                        @foreach ($areaList as $area)
                                            <option value="{{ $area->shipping_rate }}">
                                                {{ $area->area_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-2">
                                    <input type="number" step="any" name="shippingCost" class="form-control form-control-sm"
                                        id="shippingCost" value="0" oninput="countTotal()">
                                </div>
                            </div>

                            {{-- <div class="row justify-content-end align-items-center mt-2">
                                <div class="col-2">
                                    <label for="additionalCharges" class="fw-bold">Additional Charges</label>
                                </div>
                                <div class="col-2">
                                    <input type="number" name="additionalCharges" min="0" value="0" id="additionalCharges"
                                        oninput="countTotal()" class="form-control form-control-sm"
                            ...
                            <div class="row justify-content-end align-items-center mt-3">
                                <div class="col-md-2">
                                    <select class="form-select form-select-sm" id="rider_id" name="rider_id">
                                        <option value="" selected>Choose a Rider</option>
                                ...
                                <div class="col-md-2">
                                    <select class="form-select form-select-sm" id="paymentType" name="paymentType">
                            ...
                            <div class="row justify-content-end align-items-center mt-3">
                                ...
                                <div class="col-2">
                                    <input type="date" class="form-control form-control-sm" name="paymentDate"
...
@section('js')
    <script>
        ...
        function getSalesOrderInfo(sales_order_id) {
            if (!sales_order_id) {
                $('#posSaleTable thead').html(renderNormalSaleTableHead());
                $('#orderList').html('');
            } else {

                $.ajax({
                    url: "{{ route('admin.pos.getSalesOrderInfo') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                    ...
                    success: function (response) {
                        $('#posSaleTable thead').html(renderSaleOrderTableHead());
        ...
        function renderSaleOrderTableHead() {
            let tr = '<tr class="bg-light text-dark py-3">';
            tr += '<th>IMG</th>';
            tr += '<th>Product Name</th>';
            tr += '<th>Brand</th>';
            tr += '<th>PackSize/Custom</th>';
            tr += '<th>Price</th>';
            tr += '<th>QTY</th>';
            tr += '<th>Order QTY</th>';
            tr += '<th>Delivered QTY</th>';
            tr += '<th>Total</th>';
            tr += '<th>-</th>';
            tr += '</tr>';
            return tr;
        ...
        function renderNormalSaleTableHead() {
            let tr = '<tr class="bg-light text-dark py-3">';
            tr += '<th>IMG</th>';
            tr += '<th>Product Name</th>';
            tr += '<th>Brand</th>';
